...
Event* event_create_update_text(uint64_t lsn, uint64_t node_id, const char *new_text);
Event* event_create_move_current(EventType type);
Event* event_create_collapse_node(uint64_t lsn, uint64_t node_id);
Event* event_create_expand_node(uint64_t lsn, uint64_t node_id);
